...
void Company::arrangetruck(type t)
{
	switch (t)
	{
	case type::vip:
	{
		...
		while (Vtruck.dequeue(temp))
			ptemp.enqueue(temp, temp->arrangetruck());
		while (ptemp.dequeue(temp))
			Vtruck.enqueue(temp);
	}
	case type::special:
	{
...
void Company::movingtrucksToavailAcheck(DaynHour currT)
{
	...
	{
		count++;
		if (currT.DaytoHours() % 220 == 0&&count==1)
		{ 
		...
		else if (currT.DaytoHours() >= removet->Getendmoving())
		{
			...
			else if (removet->gettotaldist() >= 50000)
			{
				removet->setincheckpriorty(currT);
				removet->resetjourneys();
...
bool Company::enqueueavailtrucks(Truck* truck)
{
	...
	case type::vip:
	{
		bool e= Vtruck.enqueue(truck);
		arrangetruck(truck->gettype());
		return e;
	}
	case type::special:
	{
		bool e = Struck.enqueue(truck);
		arrangetruck(truck->gettype());
		return e;
	}
	case type::normal:
	{
		bool e = Ntruck.enqueue(truck);
		arrangetruck(truck->gettype());
		return e;
	}
	default:
...
void Company::laodingvip(DaynHour currT)
{
	...
	if (ttempv && Cvip.getcount() >= ttempv->getTcapacity())
	{
		if (isWorkingHours(currT))
		{
			if (Vtruck.isEmpty())
	...
	else if (ttempn && Cvip.getcount() >= ttempn->getTcapacity())
	{
		if (isWorkingHours(currT))
		{
			if (Ntruck.isEmpty())
	...
	else if (ttemps && Cvip.getcount() >= ttemps->getTcapacity())
	{
		if (isWorkingHours(currT))
		{
			if (Struck.isEmpty())
...
void Company::laodingnormal(DaynHour currT)
{
	...
	if (ttempn && Cnormal.getCount() >= ttempn->getTcapacity())
	{
		if (isWorkingHours(currT))
		{
			if (Ntruck.isEmpty())
	...
	else if (ttempv && Cnormal.getCount() >= ttempv->getTcapacity())
	{
		if (isWorkingHours(currT))
		{
			if (Vtruck.isEmpty())
...
void Company::laodingspecial(DaynHour currT)
{
	...
	if (ttemps && Cspecial.Getcount() >= ttemps->getTcapacity())
	{
		if (isWorkingHours(currT))
		{
			if (Struck.isEmpty())
...
void Company::loadall(string file)
{
	...
	if (infile.is_open())
	{
		int nn, ns, nv, chn, chs, chv, j, can, cas, cav, sn, ss, sv,trucks,truckc;
		infile >> nn >> ns >> nv;
	/*	infile >> sn >> ss >> sv;
		infile >> can >> cas >> cav;*/
		...
		{
			infile >> trucks >> truckc;
			Truck* t = new Truck(truckc, chn, j, trucks, type::normal);
			Ntruck.enqueue(t);
			arrangetruck(t->gettype());

		}
		for (size_t i = 0; i < ns; i++)
		{
			infile >> trucks >> truckc;
			Truck* t = new Truck(truckc, chs, j, trucks, type::special);
			Struck.enqueue(t);
			arrangetruck(t->gettype());
		...
		{
			infile >> trucks >> truckc;
			Truck* t = new Truck(truckc, chv, j, trucks, type::vip);
			Vtruck.enqueue(t);
			arrangetruck(t->gettype());
		}
		int au, max;
