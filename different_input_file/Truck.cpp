...
int Truck::arrangetruck()
{
    return 1000000/(truckcapacity*speed);
}
...
{
    float x = ceil((GetMaxDistance() / (speed) / 1.0));
    deliveryinterval = x* 2 + GettotalLtime();
    activetime+= x + GettotalLtime()*2;
